...
public:
    ...
    virtual std::string getConfiguration() const { return std::string(); }
    virtual std::string getConfigurationAsJson(const std::string &/*indent*/) const {  return std::string(); }
    virtual std::string getName() const noexcept { return "Dynamic Digraph Provider"; }
};

DynamicDiGraphProvider::~DynamicDiGraphProvider() { }
