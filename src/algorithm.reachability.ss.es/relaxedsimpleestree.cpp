...
RelaxedSimpleESTree::RelaxedSimpleESTree(unsigned int requeueLimit, double maxAffectedRatio)
...
RelaxedSimpleESTree::~RelaxedSimpleESTree()
{
    cleanup(true);
}

DiGraph::size_type RelaxedSimpleESTree::getDepthOfBFSTree() const
{
...
DiGraph::size_type RelaxedSimpleESTree::getNumReachable() const
{
...
void RelaxedSimpleESTree::run()
{
    if (initialized) {
        return;
    }

    PRINT_DEBUG("Initializing RelaxedSimpleESTree...")
...
std::string RelaxedSimpleESTree::getProfilingInfo() const
{
    std::stringstream ss;
#ifdef COLLECT_PR_DATA
...
DynamicSSReachAlgorithm::Profile RelaxedSimpleESTree::getProfile() const
{
...
void RelaxedSimpleESTree::onDiGraphSet()
{
...
void RelaxedSimpleESTree::onDiGraphUnset()
{
    DynamicSSReachAlgorithm::onDiGraphUnset();
    cleanup(true);
}

void RelaxedSimpleESTree::onVertexAdd(Vertex *v)
{
    if (!initialized) {
        return;
    }
...
void RelaxedSimpleESTree::onArcAdd(Arc *a)
{
    if (!initialized) {
        return;
    }
...
void RelaxedSimpleESTree::onVertexRemove(Vertex *v)
{
    if (!initialized) {
        return;
    }
...
void RelaxedSimpleESTree::onArcRemove(Arc *a)
{
   if (!initialized) {
        return;
    }
...
void RelaxedSimpleESTree::onSourceSet()
{
    cleanup(false);
}

bool RelaxedSimpleESTree::query(const Vertex *t)
{
...
std::vector<Arc *> RelaxedSimpleESTree::queryPath(const Vertex *t)
{
    std::vector<Arc*> path;
    if (!query(t) || t == source) {
        return path;
...
void RelaxedSimpleESTree::dumpData(std::ostream &os)
{
    if (!initialized) {
...
void RelaxedSimpleESTree::dumpTree(std::ostream &os)
{
    if (!initialized) {
        os << "uninitialized" << std::endl;
    }  else {
...
bool RelaxedSimpleESTree::checkTree()
{
...
void RelaxedSimpleESTree::rerun()
{
#ifdef COLLECT_PR_DATA
    reruns++;
#endif
...
DiGraph::size_type RelaxedSimpleESTree::process(SESVertexData *vd, bool &limitReached) {
...
void RelaxedSimpleESTree::restoreTree(SESVertexData *rd)
{
		auto n = diGraph->getSize();
...
void RelaxedSimpleESTree::cleanup(bool freeSpace)
{
    if (initialized) {
        for (auto i = data.cbegin(); i != data.cend(); i++) {
            if ((*i)) {
